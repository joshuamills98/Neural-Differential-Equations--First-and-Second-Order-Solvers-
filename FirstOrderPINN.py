    ...
    def call(self, inputs, training=True):
        if training:
            with tf.GradientTape(persistent=True) as tape:
                ...
                # Implementing approximation to ODE g(t) = u_0 +NN(t)*t
                x = self.u_0 + tf.math.multiply(inputs, x)

            # Find dg/dt
            x = tape.gradient(x, inputs)
        else:
            x = self.block(inputs)
            x = self.u_0 + tf.math.multiply(inputs, x)
        return x

# In this class we put all the data particular to our equation
...
class ODESolver(NeuralModel):
    ...
    def solve(self, epochs=400, verbose=1):
        ...
        self.fit(self.t, self.f, epochs=epochs, verbose=verbose)

    # Define the function f(x) for the solver to output dy/dx = f(x)
    def ODEFunction(self, t):
        return(-2*t + np.cos(t))
...
if __name__ == '__main__':
    ...
    print('Loss After {} Epochs = {:.2f}. Total Run-Time = {:.2f}'
          .format(n_epochs,
                  ode.evaluate(t, -t**2+np.sin(t)+2), end-start))
    plt.tight_layout()
