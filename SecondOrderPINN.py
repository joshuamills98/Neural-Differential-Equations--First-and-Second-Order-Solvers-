...
# Here we define the basic neural network structure,
# without any physics information
...
class NeuralModel(Model):
    ...
    def call(self, inputs, training=True):
        if training:
            ...
            # Return predicted boundary conditions and predicted f(x) as tuple
            return (y[0], dy_dx[0], d2y_dx2)
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        y0_pred, dy_dx0_pred, d2y_dx2_pred = y_pred
        self.loss = tf.reduce_mean(tf.math.square(d2y_dx2_pred-fx)) + \
            tf.reduce_mean(tf.math.square(y0_pred-y0_true)) + \
            tf.reduce_mean(tf.math.square(dy_dx0_pred-dy_dx0_true))

        return self.loss

    # Overwright Keras training step to suit ODE Model
    def train_step(self, data):

        # Pass to the NN input domain, f(x),
        # and the two boundary conditions y is a tuple
        x, y_true = data
...
class ODESolver(NeuralModel):
    ...
    # Custom solver is implemented since using .fit not working
    def solve(self, epochs=400, batch_size=1, verbose=1):
        for i in range(epochs):
            loss = self.train_step([self.x, self.y_true])
            if i % 100 == 0:
                print('Epoch = {}. Loss = {}'.format(i, loss['loss']))

    # Define the function f(x) for the solver to output d^2y/dx^2 = f(x)
    def ODEFunction(self, x):
        return(-np.sin(x)+x)
...
if __name__ == '__main__':
    ...
    ax[1].set_xlabel('x')
    ax[1].set_ylabel('y')
    print('Loss After {} Epochs = {:.2f}. Total Run-Time = {:.2f}'
          .format(n_epochs, ode.loss.numpy(), end-start))
